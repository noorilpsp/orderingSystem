/**
 * Run drizzle/0032_promotions_display_order.sql (no psql required).
 * Loads .env.local via Next.js env config.
 *
 * Usage: npm run db:migrate:0032
 */
...
async function run() {
  const sql = neon(url);
  const migrationPath = join(process.cwd(), "drizzle/0032_promotions_display_order.sql");
