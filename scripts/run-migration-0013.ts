/**
 * Run drizzle/0013_pos_idempotency_keys.sql (no psql required).
 * Loads .env.local via Next.js env config.
 *
 * Usage: npm run db:migrate:0013
 */
...
async function run() {
  const sql = neon(url);
  const migrationPath = join(process.cwd(), "drizzle/0013_pos_idempotency_keys.sql");
