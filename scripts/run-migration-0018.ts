/**
 * Run drizzle/0018_loyalty_rewards.sql (no psql required).
 * Loads .env.local via Next.js env config.
 *
 * Usage: npm run db:migrate:0018
 */
...
async function run() {
  const sql = neon(url);
  const migrationPath = join(process.cwd(), "drizzle/0018_loyalty_rewards.sql");
  ...
  for (const stmt of stmts) {
    try {
      ...
      // Idempotent re-runs: type/table/index already exists.
      if (
