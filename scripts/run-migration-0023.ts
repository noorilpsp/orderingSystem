/**
 * Run drizzle/0023_merchant_features.sql (no psql required).
 * Loads .env.local via Next.js env config.
 *
 * Usage: npm run db:migrate:0023
 */
...
async function run() {
  const sql = neon(url);
  const migrationPath = join(process.cwd(), "drizzle/0023_merchant_features.sql");
