/**
 * Run drizzle/0020_staff_push_subscriptions.sql (no psql required).
 * Loads .env.local via Next.js env config.
 *
 * Usage: npm run db:migrate:0020
 */
...
async function run() {
  const sql = neon(url);
  const migrationPath = join(process.cwd(), "drizzle/0020_staff_push_subscriptions.sql");
